...
# ── 16x16 display face (title logos) ─────────────────────────────────
#
# A second weight, hand-plotted rather than scaled from the 8x8 set:
# doubling an 8px face gives 2px-thick strokes on a 16px body and
# monospaced cells, which is what made the old logo look like a
# placeholder. This one has a 3px stem at a 14px cap height and real
# per-letter widths, so the codegen can space and centre it properly.
#
# Rows are the glyph's 14 cap rows; the cell adds a blank row above
# and below. '#' is ink.
TITLE_FACE = {
    " ": [
        "......",
...
def title_font():
    """(recode[128], widths, bitmaps) for the 16x16 display face.

    Each glyph is 16 rows x 2 bytes, MSB-left — the same 1bpp shape as
    the 8px font, just twice as wide, so one parser reads both.
    """
    order = [" "] + sorted(k for k in TITLE_FACE if k != " ")
    recode = [0] * 128
    widths, bitmaps = [], []
    for idx, ch in enumerate(order):
        rows = TITLE_FACE[ch]
        ink = max((len(r.rstrip(".")) for r in rows), default=0)
        cell = []
        for y in range(16):
            src = rows[y - 1] if 1 <= y <= 14 else ""
            hi = lo = 0
            for x, c in enumerate(src):
                if c == "#":
                    if x < 8:
                        ...
                        lo |= 0x80 >> (x - 8)
            cell += [hi, lo]
        widths.append(ink + 2 if ink else 6)   # +2px sidebearing
        bitmaps.append(cell)
        if ch != " ":
            recode[ord(ch)] = idx
            if ch.isalpha():
                recode[ord(ch.lower())] = idx
    return recode, widths, bitmaps
...
def emit_vwf_font(hdr, pool_base, pool_size):
    ...
    trecode, twidths, tbitmaps = title_font()
    trecode_c = "\n".join(
        "    " + ", ".join(f"{v:3d}" for v in trecode[i:i + 16])
        + f",  // 0x{i:02X}-0x{i + 15:02X}"
        for i in range(0, 128, 16))
    twidth_c = "\n".join(
        "    " + ", ".join(str(w) for w in twidths[i:i + 12]) + ","
        for i in range(0, len(twidths), 12))
    torder = [" "] + sorted(k for k in TITLE_FACE if k != " ")
    tglyph_c = "\n".join(
        "    " + ", ".join(f"0x{b:02X}" for b in tbitmaps[i])
        + f",  // {i}: {torder[i] if torder[i] != ' ' else 'SPC'}"
        for i in range(len(tbitmaps)))
...
const uint8_t title_widths[{len(twidths)}] = {{
{twidth_c}
}};

const uint8_t title_bitmaps[{len(tbitmaps) * 32}] = {{
{tglyph_c}
}};
""")
...
def main():
    ...
    ui[40] = arrow_right()
    _acols = [x for x in range(8) if any(ui[40][y][x] for y in range(8))]
    arrow_x0, arrow_x1 = (min(_acols), max(_acols)) if _acols else (0, 7)
    ui[41] = checkmark()
 ...
 * the cell. */
#define UI_TILE_ARROW_INK_X0 {arrow_x0}
#define UI_TILE_ARROW_INK_X1 {arrow_x1}
